...
public class EntityArticFox extends EntityTameable {
	private static final DataParameter<Float> DATA_HEALTH_ID = EntityDataManager.<Float>createKey(EntityArticFox.class,
			DataSerializers.FLOAT);
	
	private static final DataParameter<Boolean> BEGGING = EntityDataManager.<Boolean>createKey(EntityArticFox.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> COLLAR_COLOR = EntityDataManager
			.<Integer>createKey(EntityArticFox.class, DataSerializers.VARINT);
	public static final ResourceLocation LOOT_ARTIC_FOX = new ResourceLocation(Reference.ID, "entities/artic_fox");
	...
	public EntityArticFox(World worldIn) {
		super(worldIn);
	...
	public static void registerFixesFox(DataFixer fixer) {
		EntityLiving.registerFixesMob(fixer, "ArticFox");
	}
	...
	/**
	 * Used when calculating the amount of shading to apply while the Fox is
	 * wet.
	 */
	@SideOnly(Side.CLIENT)
	public float getShadingWhileWet(float p_70915_1_) {
		return 0.75F + (this.prevTimeFoxIsShaking + (this.timeFoxIsShaking - this.prevTimeFoxIsShaking) * p_70915_1_)
				/ 2.0F * 0.25F;
	}

	@SideOnly(Side.CLIENT)
	public float getShakeAngle(float p_70923_1_, float p_70923_2_) {
		float f = (this.prevTimeFoxIsShaking + (this.timeFoxIsShaking - this.prevTimeFoxIsShaking) * p_70923_1_
				+ p_70923_2_) / 1.8F;
	...
	/**
	 * Determines whether this Fox is angry or not.
	 */
	...
	/**
	 * Sets whether this Fox is angry or not.
	 */
	...
	public EntityArticFox createChild(EntityAgeable ageable) {
		EntityArticFox entityArticFox = new EntityArticFox(this.worldObj);
		UUID uuid = this.getOwnerId();

		if (uuid != null) {
			entityArticFox.setOwnerId(uuid);
			entityArticFox.setTamed(true);
		}

		return entityArticFox;
	}
	...
	public boolean canMateWith(EntityAnimal otherAnimal) {
		if (otherAnimal == this) {
			...
			return false;
		} else if (!(otherAnimal instanceof EntityArticFox)) {
			return false;
		} else {
			EntityArticFox EntityArticFox = (EntityArticFox) otherAnimal;
			return !EntityArticFox.isTamed() ? false
					: (EntityArticFox.isSitting() ? false : this.isInLove() && EntityArticFox.isInLove());
		}
	}
	...
	public boolean shouldAttackEntity(EntityLivingBase p_142018_1_, EntityLivingBase p_142018_2_) {
		if (!(p_142018_1_ instanceof EntityCreeper) && !(p_142018_1_ instanceof EntityGhast)) {
			if (p_142018_1_ instanceof EntityArticFox) {
				EntityArticFox EntityArticFox = (EntityArticFox) p_142018_1_;

				if (EntityArticFox.isTamed() && EntityArticFox.getOwner() == p_142018_2_) {
					return false;
				}
			}
	...
	static class FoxBeg extends EntityAIBase {
		private final EntityArticFox theFox;
		private EntityPlayer thePlayer;
		private final World worldObject;
		private final float minPlayerDistance;
		private int timeoutCounter;

		public FoxBeg(EntityArticFox Fox, float minDistance) {
			this.theFox = Fox;
			this.worldObject = Fox.worldObj;
		...
		public boolean shouldExecute() {
			this.thePlayer = this.worldObject.getClosestPlayerToEntity(this.theFox, (double) this.minPlayerDistance);
			return this.thePlayer == null ? false : this.hasPlayerGotMeatInHand(this.thePlayer);
		}
		...
		public boolean continueExecuting() {
			return !this.thePlayer.isEntityAlive() ? false
					: (this.theFox.getDistanceSqToEntity(
							this.thePlayer) > (double) (this.minPlayerDistance * this.minPlayerDistance) ? false
									: this.timeoutCounter > 0 && this.hasPlayerGotMeatInHand(this.thePlayer));
		}
		...
		public void startExecuting() {
			this.theFox.setBegging(true);
			this.timeoutCounter = 40 + this.theFox.getRNG().nextInt(40);
		}
		...
		public void updateTask() {
			this.theFox.getLookHelper().setLookPosition(this.thePlayer.posX,
					this.thePlayer.posY + (double) this.thePlayer.getEyeHeight(), this.thePlayer.posZ, 10.0F,
					(float) this.theFox.getVerticalFaceSpeed());
			--this.timeoutCounter;
		}
		 ...
		 */
		private boolean hasPlayerGotMeatInHand(EntityPlayer player) {
			for (EnumHand enumhand : EnumHand.values()) {
				ItemStack itemstack = player.getHeldItem(enumhand);

				if (itemstack != null) {
					if (this.theFox.isTamed() && itemstack.getItem() == Items.BONE) {
						return true;
					}

					if (this.theFox.isBreedingItem(itemstack)) {
						return true;
					}
				}
			}
